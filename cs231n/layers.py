...
def affine_forward(x, w, b):
    ...
    # Reshape x to a 2D matrix of shape (N, D)
    N = x.shape[0]
    D = np.prod(x.shape[1:])
    x_reshaped = x.reshape(N,D)

    # Compute the forward pass: out = x_reshaped * w + b
...
def affine_backward(dout, cache):
    ...
    # Compute gradients
    dx = np.dot(dout, w.T).reshape(x.shape)  # Reshape back to the shape of x
...
def relu_backward(dout, cache):
    ...
    # Compute gradient: set dx to dout where x > 0, otherwise dx = 0
    dx = dout * (x > 0)
...
def svm_loss(x, y):
    ...
    # Assigning the number of classes and examples
    N = x.shape[0]
    C = x.shape[1]

    # Compute the correct scores
    correct_class_scores = x[np.arange(N),y]

    # Margins for all classes and delta = 1
    margins = np.maximum(0, x - correct_class_scores[:,np.newaxis] + 1) 
    margins[np.arange(N), y] = 0
    
    # Compute the loss as the mean over all examples
...
def softmax_loss(x, y):
    ...
    dx /= N  # Average the gradient over the batch
